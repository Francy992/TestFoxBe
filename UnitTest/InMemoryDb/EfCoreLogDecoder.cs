...
namespace UnitTest.InMemoryDb
{
    ...
    internal class EfCoreLogDecoder
    {
        ...
        private EfCoreLogDecoder(Match matchedParam)
        {
            ...
            _paramTypes = matchedParam.Groups[4].Captures.Select(x => x.Value).ToArray();
        }
        ...
        public static string DecodeMessage(LogOutput log)
        {
            ...
            var parametersIndex = messageLines[0].IndexOf(ParameterStart, StringComparison.InvariantCultureIgnoreCase);
            if (parametersIndex <= 0)
                return log.Message;

            var decodedMatches = ParamRegex
                .Matches(messageLines[0][(parametersIndex + ParameterStart.Length)..])
                .Select(x => new EfCoreLogDecoder(x))
                .ToList();
